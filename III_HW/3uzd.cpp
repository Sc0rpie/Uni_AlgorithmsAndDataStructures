...
void writeStateData(State state)
{
    ...
    out << "Olen Rackauskas. 3 grupe, 2 pogrupis." << endl << endl;
    out << "SALYGA: " << "Psichologo konsultacijas teikia keli specialistai, dalis ju (n) formuoja terapines grupes is k zmoniu, kiti (m) priima pacientus individualiai. Grupines terapijos trukme zymima GrT, o privacios terapijos trukme zymima InT (laikas valandomis). Kiekvienas naujai atejes pacientas (naujo paciento pasirodymo tikimybe p1, pakartotinai pacientai ateina po tam tikro laiko d) gali pasirinkti ar apsilankyti pas psichologa, teikianti individualia paslauga, ar pas grupines konsultacijas. Kol pas psichologa teikianti grupine konsultacija susirenka grupe i ja pretenduojantys pacientai turi laukti. Pakartotinai ateje pacientai, pas savo psichologa gali rinktis ir mazesnem grupem, taciau negali eiti kartu su naujokais. Yra pastebeta, kad individualiai priimami pacientai sveiksta greiciau, nei dirbantys grupeje (jiems reikia maziau konsultaciju), taciau pasitaiko isimciu, del socialinio bendravimo faktoriaus, kai kurie pacientai (tikimybe p2) pasveiksta h procentu greiciau, nei konsultuojami individualiai. Individuli konsultacija kainuoja InK litu, grupine GrK litu. Reikia ivertinti, koki terapijos metoda turetu pasirinkti pacientas optimizuodamas savo isteklius, bei kiek ir kokiu specialistu turetu samdyti sveikatos prieziuros istaiga.";
    out << "DALIS 1. Ivesti rodikliai." << endl;
    out << "Proceso trukme:\t\t\t\t\t\t\t" << setw(3) << state.sim_hours << " val." << endl;
    out << "Privaciu psichologu skaicius:\t\t\t\t\t" << setw(3) << state.m << " privatus psicholog." << endl;
    out << "Grupiniu psichologu skaicius:\t\t\t\t\t" << setw(3) << state.n << " grupes psicholog." << endl;
    out << "Privacios terapijos trukme:\t\t\t\t\t" << setw(3) << state.InT << " val." << endl;
    out << "Grupines terapijos trukme:\t\t\t\t\t" << setw(3) << state.GrT << " val." << endl;
    out << "Pacientu skaicius grupeje (nauji pacientai):\t\t\t" << setw(3) << state.k << " pacient." << endl;
    out << "Pacientu skaicius grupeje (pakartotinai ateinantys pacientai):\t" << setw(3) << state.nK << " pacient." << endl;
    out << "Po kiek laiko grizta pacientai:\t\t\t\t\t" << setw(3) << state.d << " val." << endl;
    out << "Tikimybe, kad ateis naujas pacientas:\t\t\t\t" << setw(3) << state.p1 << " %." << endl;
    out << "Tikimybe, kad pacientas pasveiks greiciau:\t\t\t" << setw(3) << state.p2 << " %." << endl;
    out << "Tikimybe ant kiek greiciau pasveiks pacientas:\t\t\t" << setw(3) << state.h << " %." << endl;
    out << "Tikimybe, kad pacientas pasirinks privacia terapija:\t\t" << setw(3) << state.p3 << " %." << endl;
    out << "Kiek kartu reikia nueiti i grupines terapijas:\t\t\t" << setw(3) << state.GrN << " kart." << endl;
    out << "Kiek kartu reikia nueiti i privacias terapijas:\t\t\t" << setw(3) << state.InN << " kart." << endl;
    out << "Privacios terapijos kaina:\t\t\t\t\t" << setw(3) << state.InK << " eur." << endl;
    out << "Grupines terapijos kaina:\t\t\t\t\t" << setw(3) << state.GrK << " eur." << endl
         << endl;
...
void getWaitAndReturnList(State state, Clinic clinic)
{
    ...
    out << "Atejusiu pacientu skaicius \t\t\t\t= " << setw(3) << patientID << '.' << endl;
    out << "Pasveikusiu pacientu skaicius \t\t\t\t= " << setw(3) << clinic.isWell << '.' << endl;
    out << "Laukianciu pacientu skaicius (grupines terapijos) \t= " << setw(3) << clinic.groupWaiting.size() << '.' << endl;
    out << "Laukianciu pacientu skaicius (privacios terapijos) \t= " << setw(3) << clinic.indivWaiting.size() << '.' << endl;
    ...
    out << "Sugriztanciu pacientu skaicius (grupines terapijos) \t= " << setw(3) << gr << '.' << endl;
...
Clinic checkIndividualPatients(State state, Clinic clinic, int time)
{
    ...
    if (size > 0)
    {
        ...
        {
            out << "\tAtvyko pacientas P" << clinic.patients.front().ID << " ir nori privacios terapijos." << endl;
            // bool wasAdded = false;
            ...
            if (!wasAdded)
            {
                out << "\tP" << clinic.patients.front().ID << " atsistojo i eile." << endl;
    ...
    if (size > 0)
    {
        ...
        if (clinic.returningP.front().isIndividual)
        {
            ...
            if (clinic.returningP.front().whenToReturn == time)
                out << "\tAtvyko pacientas P" << clinic.returningP.front().ID << "(pakartotinai) ir nori privacios terapijos." << endl;
            for (int i = 0; i < state.m; i++)
            {
            ...
            {
                out << "\tP" << clinic.returningP.front().ID << " atsistojo i eile." << endl;
...
Clinic checkGroupPatients(State state, Clinic clinic, int time)
{
        ...
        for (int i = 0; i < size; i++)
        {
            for (int i = maxGroup(state, clinic, true); i < state.n; i++)
            {
                if (clinic.groupTherapies[i].patients.size() < state.k && clinic.groupWaiting.front().firstTime == true && clinic.groupTherapies[i].isForFirstTime)
                {
                    ...
                    out << "\tP" << clinic.groupWaiting.front().ID << " yra priskirtas prie grupes G" << i + 1 << ". Jos dydis = " << clinic.groupTherapies[i].patients.size() << " pacient." << endl;
    ...
    if (size > 0)
    {
        ...
        {
            out << "\tAtvyko pacientas P" << clinic.patients.front().ID << " ir nori grupines terapijos." << endl;
            int id;
            for (int i = maxGroup(state, clinic, true); i < state.n; i++)
            {
                if (clinic.groupTherapies[i].patients.size() < state.k && clinic.patients.front().firstTime == true && clinic.groupTherapies[i].isForFirstTime)
                {
                    ...
                    clinic.groupTherapies[i].isForFirstTime = true;
                    out << "\tP" << clinic.patients.front().ID << " yra priskirtas prie grupes G" << i + 1 << ". Jos dydis = " << clinic.groupTherapies[i].patients.size() << " pacient." << endl;
                    clinic.patients.pop();
                    wasAdded = true;
                    // out << "Assigned to group therapy " << i + 1 << endl;
                    // out << "Grupes " << i+1 << " dydis = " << clinic.groupTherapies[i].patients.size() << endl;
...
Revenue simulate(State state)
{
    ...
    while (time <= state.sim_hours)
    {
        ...
        for (int i = 0; i < state.m; i++) // individual therapies
        {
            ...
            {
                out << "\tPrivati terapija su P" << clinic.individualTherapies[i].patient.ID << " prasidejo pas privatu psichologa " << i + 1 << '.' << endl;
            ...
            {
                out << "\tPrivati terapija su P" << clinic.individualTherapies[i].patient.ID << " pasibaige pas privatu psichologa " << i + 1 << '.' << endl;
                    ...
                    for (int j = maxGroup(state, clinic, false); j < state.n; j++)
                    {
                        ...
                        for (int i = 0; i < size; i++)
                        {
                            ...
                            if ((clinic.groupTherapies[j].patients.size() == 0 && clinic.returningP.front().whenToReturn == time) || (clinic.groupTherapies[j].patients.size() < state.nK && clinic.groupTherapies[j].isForFirstTime == false && clinic.returningP.front().whenToReturn == time))
                            {
                                ...
                                out << "\tP" << clinic.returningP.front().ID << " priskiriamas prie grupes G" << j + 1 << ". Jos dydis: " << clinic.groupTherapies[j].patients.size() << " pacient." << endl;
                                clinic.returningP.pop();
                                wasAdded = true;
        ...
        for (int i = 0; i < state.n; i++) // group therapies
        {
            ...
            {
                out << "\tGrupine G" << i+1 << " terapija prasidejo." << endl;
            ...
            {
                out << "\tGrupine G" << i+1 << " terapija pasibaige." << endl;
    ...
    out << "Grupiniu terapiju pelnas \t\t\t\t= " << setw(3) << revenue.GrR << " eur." << endl;
    out << "Privaciu terapiju pelnas \t\t\t\t= " << setw(3) << revenue.InR << " eur." << endl << endl;
